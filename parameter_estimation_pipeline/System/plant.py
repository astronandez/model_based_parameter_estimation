...
def run_simulation(Q_scale, R_scale, num_simulations=1000, num_steps=100, dt=0.1):
    ...
    for i in range(num_simulations):
        ...
        # Initialize the SimpleHarmonicOscillator
        oscillator = SimpleHarmonicOscillator(λ, k, b, dt, H, Q, R)
        
        # Initialize the Plant with the system
        x0 = np.array([0.0, 0.0]).reshape(2, 1)  # Initial state
        plant = Plant(oscillator, x0, noisy=True)
...
def plot_combined_results(results, dt):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join('output', 'plant_simulation_combined.png'))
    plt.show()
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join('output', 'plant_simulation_mse_combined.png'))
    plt.show()
    plt.close()
...
if __name__ == "__main__":
    ...
    print(f"Plant class Monte Carlo tests ({num_simulations} simulations) completed for all Q and R combinations.")
